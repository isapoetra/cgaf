/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -  */
//Original Source code :http://dromaeo.com/tests/dromaeo-core-eval.html
function AES() {
  /*
   * AES Cipher function: encrypt 'input' with Rijndael algorithm
   * 
   * takes byte-array 'input' (16 bytes) 2D byte-array key schedule 'w' (Nr+1 x
   * Nb bytes)
   * 
  ...
  function Cipher(input, w) { 
    ...
    var Nr = w.length
        / Nb - 1; // no of rounds: 10/12/14 for 128/192/256-bit keys

    var state = [
        [], [], [], []
    ]; // initialise 4xNb byte-array 'state' with input [Â§3.4]
  ...
  function MixColumns(s, Nb) { // combine bytes of each col of state S [Â§5.1.3]
    for ( var c = 0; c < 4; c++) {
      ...
      s[0][c] = b[0]
          ^ a[1] ^ b[1] ^ a[2] ^ a[3]; // 2*a0 + 3*a1 + a2 + a3
      s[1][c] = a[0]
          ^ b[1] ^ a[2] ^ b[2] ^ a[3]; // a0 * 2*a1 + 3*a2 + a3
      s[2][c] = a[0]
          ^ a[1] ^ b[2] ^ a[3] ^ b[3]; // a0 + a1 + 2*a2 + 3*a3
      s[3][c] = a[0]
          ^ b[0] ^ a[1] ^ a[2] ^ b[3]; // 3*a0 + a1 + a2 + 2*a3
    }
    return s;
  }
  ...
  /*
   * if escCtrlChars()/unescCtrlChars() still gives problems, use
   * encodeBase64()/decodeBase64() instead
   */
  ...
  function encodeBase64(str) { // http://tools.ietf.org/html/rfc4648
    ...
    do { // pack three octets into four hexets
      ...
      enc += b64.charAt(h1)
          + b64.charAt(h2) + b64.charAt(h3) + b64.charAt(h4);
    } while (i < str.length);
  ...
  function encodeUTF8(str) { // encode multi-byte string into utf-8 multiple single-byte characters 
    ...
    str =
        str.replace(/[\u0800-\uffff]/g, // U+0800 - U+FFFF = 3-byte chars
        function(c) {
          var cc = c.charCodeAt(0);
  ...
  return {
     ...
     * 'Counter' mode of operation - see
     * http://csrc.nist.gov/publications/nistpubs/800-38a/sp800-38a.pdf for each
     * block - outputblock = cipher(counter, key) - cipherblock = plaintext xor
     * outputblock
     */
    Encrypt : function(plaintext, password, nBits) {
      ...
      var blockCount = Math.ceil(plaintext.length
          / blockSize);
      ...
      for ( var b = 0; b < blockCount; b++) {
        ...
        var blockLength =
            b < blockCount - 1 ? blockSize : (plaintext.length - 1)
                % blockSize + 1;

        var ct = '';
        for ( var i = 0; i < blockLength; i++) { // -- xor plaintext with ciphered counter byte-by-byte --
          var plaintextByte = plaintext.charCodeAt(b
              * blockSize + i);
          var cipherByte = plaintextByte
              ^ cipherCntr[i];
     ...
     * 
     * for each block - outputblock = cipher(counter, key) - cipherblock =
     * plaintext xor outputblock
     */
    Decrypt : function(ciphertext, password, nBits) {
      ...
      for ( var b = 1; b < ciphertext.length; b++) {
        ...
        for ( var i = 0; i < ciphertext[b].length; i++) {
          ...
          var plaintextByte = ciphertextByte
              ^ cipherCntr[i];
