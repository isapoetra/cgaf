...
;(function(factory) {
    ...
    function purl( url, strictMode ) {
        ...
        return {
            ...
            setSegment: function(param) {
                if (typeof (param) === 'string') {
                    this.data.seg.path = param.split('/');
                }else if (typeof (param) === 'object') {
                    this.data.seg.path=param;
                }
                return this;
            },
            addSegment : function(segment) {
                if (typeof (segment) !== 'object') {
                    var paths = segment.toString().split('/')
                    for ( var i in paths) {
                        this.data.seg.path.push(paths[i]);
                    }
                }else{
                    for ( var i in segment) {
                        this.data.seg.path.push(segment[i]);
                    }
                }
                return this;
            },
            //deprecated please use addSegment
            addPath: function(path) {
               return this.addSegment(path);
            },
            setParam : function(param) {
                if (typeof (param) === 'string') {
                    var tmp = [];
                    param.replace(querystring_parser, function($0, $1, $2) {
                        if ($1) {
                            tmp[$1] = $2;
                        }
                    });
                    for ( var i in tmp) {
                       this.data.param.query[i] = tmp[i];
                    }
                } else if (typeof (param) === 'object') {
                    for ( var i in param) {
                        this.data.param.query[i] = param[i];
                    }
                }
                return this;
            },
            toString : function() {
                var param = [];
                for ( var i in this.data.param.query) {
                    param.push(i + '=' + this.data.param.query[i]);
                }
                var path = [];
                for(var i in this.data.seg.path) {
                    if (this.data.seg.path[i].toString() !== '') {
                        path.push( this.data.seg.path[i]);
                    }
                }
