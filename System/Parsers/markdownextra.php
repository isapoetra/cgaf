...
class MarkdownExtra extends Markdown
{
    ...
    function _hashHTMLBlocks_inMarkdown($text, $indent = 0, $enclosing_tag_re = '', $span = false)
    {
        ...
        do {
            ...
            #
            else if (preg_match('{^<(?:' . $this->block_tags_re . ')\b}', $tag) || (preg_match('{^<(?:' . $this->context_block_tags_re . ')\b}', $tag) && preg_match($newline_before_re, $parsed) && preg_match($newline_after_re, $text))) {
    ...
    function doHeaders($text)
    {
        ...
        $text = preg_replace_callback('{
            ...
            &$this,
            '_doHeaders_callback_setext'), $text);
        # atx-style headers:
        #	# Header 1        {#header1}
        #	## Header 2       {#header2}
...
.* [|] .* \n		# Row content
    ...
    function _doTable_leadingPipe_callback($matches)
    {
        ...
        return $this->_doTable_callback(array(
            $matches[0],
            $head,
            $underline,
            $content));
...
\1 [ ]* \n
            ...
            &$this,
            '_doFencedCodeBlocks_callback'), $text);
        return $text;
    }
    ...
    function _doFencedCodeBlocks_callback($matches)
    {
        ...
        $codeblock = preg_replace_callback('/^\n+/', array(
            &$this,
            '_doFencedCodeBlocks_newlines'), $codeblock);
...
# by non-indented content
    ...
    function appendFootnotes($text)
    {
        ...
        $text = preg_replace_callback('{F\x1Afn:(.*?)\x1A:}', array(
            &$this,
            '_appendFootnotes_callback'), $text);
        if (!empty($this->footnotes_ordered)) {
            ...
            while (!empty($this->footnotes_ordered)) {
                ...
                $footnote = preg_replace_callback('{F\x1Afn:(.*?)\x1A:}', array(
                    &$this,
                    '_appendFootnotes_callback'), $footnote);
	...
	^[ ]{0,' . $less_than_tab . '}\*\[(.+?)\][ ]?:	# abbr_id = $1
            ...
            &$this,
            '_stripAbbreviations_callback'), $text);
        return $text;
    }
    ...
    function doAbbreviations($text)
    {
        ...
        if ($this->abbr_word_re) {
            ...
            $text = preg_replace_callback('{' . '(?<![\w\x1A])' . '(?:' . $this->abbr_word_re . ')' . '(?![\w\x1A])' . '}', array(
                &$this,
                '_doAbbreviations_callback'), $text);
        }
        return $text;
    }
