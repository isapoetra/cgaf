...
class Db extends BaseACL {
	...
	function revoke($id, $group, $access = "view", $userid = null) {
		...
		if ($o) {
			$o->privs &= ~$access;
			if ($o->privs == 0) {
				$this->_q->clear()->setMode("delete")->addTable("user_privs")->where("user_id=" . (int) $userid)->where("app_id=" . $this->_q->quote($this->_appId))->where("object_id=" . $this->_q->quote($id))
						->where("object_type=" . $this->_q->quote($group))->exec();
			} else {
				$this->_q->clear()->addTable("user_privs")->Update("privs", $o->privs, "=", true)->where("user_id=" . (int) $userid)->where("app_id=" . $this->_q->quote($this->_appId))->where("object_id=" . $this->_q->quote($id))
						->where("object_type=" . $this->_q->quote($group))->exec();
			}
		}
	}
	...
	function getUserInRole($rolename, $byName = true) {
		...
		$q->join("roles", "r", "ur.role_id=r.role_id and r.app_id=" . $this->_q->quote($this->_appId));
		$q->join('users', 'u', 'ur.user_id=u.user_id and ur.app_id=' . $this->_q->quote($this->_appId));
