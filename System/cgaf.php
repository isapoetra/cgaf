...
final class CGAF {
	...
	private static function handleAssetNotFound() {
        ...
        $f = htmlentities(Utils::filterXSS(implode('/',$url['path'])));
	...
	/*public static function Using($namespace = null, $throw = true) {
	    return NSHelper::Using($namespace, $throw);
	}*/
	...
	public static function Using($namespace = null, $throw = true) {
		//return NamespaceHelper::Using($namespace, $throw);
		if ($namespace === null) {
	...
	public static function LoadClass($className, $throw = true) {
		...
		foreach ($nspath as $p) {
			...
			foreach ($namespaces as $namepart) {
				...
				$fnload = self::toDirectory($p . $current . "__init.php");
...
class NSHelper {
	private static $_classPath = array();
	private static $_cachedFiles = array();
	private static $_namespaces = array();

	public static function getDirFiles($dir, $base = null, $recurse = true,
			$match = null) {
		$dir = realpath($dir);
		$files = array();
		if (!is_dir($dir)) {
		...
		$dir = rtrim($dir, ' ' . DS) . DS;
		$base = $base ? rtrim($base, ' ' . DS) . DS : null;
		$dh = opendir($dir);
		if (!$dh) {
			return $files;
		}
		while (false !== ($filename = readdir($dh))) {
			...
			if (is_file($dir . DS . $filename)) {
				...
				$childs = self::getDirFiles($dir . DS . $filename,
						$base . $filename, true, $match);
	...
	public static function addClassPath($nsName, $path, $first = true,
		...
		//$files = self::getDirFiles($path, $path, $recurse, '/\.php$/i');
		$path .= DS;
		$nsName = str_replace('.', DS, strtolower($nsName));
		if (!isset(self::$_classPath[$nsName])) {
			self::$_classPath[$nsName] = array();
		...
		//self::$_cachedFiles[$nsName] = $files;
		//print_r(self::$_cachedFiles);
	}

	public static function loadClass($class) {
		if (!strstr($class, NSS)) {
			$class = 'System' . NSS . $class;
		}
		self::Using($class);
	}

	private static function _toNS($namespace) {
		if (substr($namespace, strrpos($namespace, '.') + 1) === 'php') {
			$namespace = realpath($namespace);
			if ($namespace) {
				foreach (self::$_classPath as $k => $v) {
					foreach ($v as $kk => $vv) {
						if (substr($namespace, 0, strlen($vv)) === $vv) {
							$namespace = str_replace($vv, $k . '.', $namespace);
							break;
						}
					}
				}
	...
	private static function getFileOfNS($namespace) {
		$namespace = str_replace('.', NSS, $namespace);
		$first = strtolower(substr($namespace, 0, strpos($namespace, NSS)));
		$last = substr($namespace, strrpos($namespace, NSS) + 1);
		$cc = count_chars($namespace, 1);
		$xpath = '';
		if (isset($cc[ord(NSS)]) && $cc[ord(NSS)] > 1) {
			$l = strlen($namespace)
					- (strlen($namespace)
							- (strrpos($namespace, NSS) - strlen($first)));
			$xpath = str_replace(NSS, DS,
					trim(substr($namespace, strlen($first) + 1, $l), ' ' . NSS));
		}
		$paths = isset(self::$_classPath[$first]) ? self::$_classPath[$first]
				: self::$_classPath['system'];
		$retval = array();
		$falts = array();
		foreach ($paths as $path) {
			$falts[] = $path . $xpath . DS . $last . CGAF_CLASS_EXT;
			$falts[] = $path . $xpath . DS . strtolower($last) . CGAF_CLASS_EXT;
			$falts[] = $path . $xpath . DS . $last;
			$falts[] = $path . $xpath . DS . strtolower($last);
		}
		foreach ($falts as $f) {
			if (is_file($f)) {
				$retval[] = $f;
				...
				$retval = self::getDirFiles($f, $f, false,
						"/\\" . CGAF_CLASS_EXT . "/i");
				break;
			}
		}
	...
	public static function Using($namespace, $throw = true) {
		if (!$namespace)
			return self::$_namespaces;
		if ($namespace === (array) $namespace) {
			$retval = array();
			foreach ($namespace as $ns) {
				$retval[$ns] = self::Using($ns, false);
			}
			return $retval;
		}
		$nsnormal = self::_toNS($namespace);
		...
		if (substr($namespace, strlen($namespace) - 4) === CGAF_CLASS_EXT) {
			$namespace = realpath($namespace);
			if (!in_array($namespace, self::$_namespaces[$nsnormal])) {
				self::$_namespaces[$nsnormal][] = $namespace;
				require $namespace;
			...
			return true;
		} elseif (is_dir($namespace)) {
			return self::Using(
					self::getDirFiles($namespace, $namespace, false,
							"/\\" . CGAF_CLASS_EXT . "/i"));
		} else {
			if ($nsnormal[strlen($nsnormal) - 1] === '*') {
				$files = self::getFileOfNS(
						substr($nsnormal, 0, strrpos($nsnormal, '.')));
				return self::Using($files);
			}
			$files = self::getFileOfNS($namespace);
...
//spl_autoload_register('NSHelper::loadClass');
