...
final class CGAF {
	...
	static function Initialize() {
		...
		if (!defined("CGAF_APP_PATH")) {
			define("CGAF_APP_PATH", self::getConfig('cgaf.app.defaultPath', CGAF_PATH . "Applications"));
		}
		$paths = self::getConfiguration()->getConfigs('cgaf.app.paths', array());
		if (!in_array(CGAF_APP_PATH, $paths)) {
			$paths[] = CGAF_APP_PATH;
			self::getConfiguration()->setConfig('cgaf.app.paths', $paths);
		}
		...
		if ($debugMode) {
			if (!defined('CGAF_DEBUG')) {
				if (isset($_SERVER['REMOTE_ADDR'])) {
					...
					if ($dt === md5(self::getConfig('cgaf.devtoken'))) {
		...
		if (CGAF_DEBUG) {
			define('CGAF_DEV_PATH',
				self::getConfig("cgaf.devpath", realpath(dirname(__FILE__) . DS . '../../DevFiles/')) . DS);
		...
		self::addClassPath('system', CGAF_SHARED_PATH . 'classes' . DS, false);
...
'<img src="/assets/images/user-notallow.png"/>');*/
	...
	protected static function pathToLive($path) {
		...
		/*if (Strings::BeginWith($path, $cpath)) {
