...
/*
 * Dio - PHP OpenDocument Generator Copyright (C) 2008-2009 Étienne BERSAC
 * <bersace03@gmail.com> This program is free software: you can redistribute it
 * and/or modify it under the terms of the GNU Affero General Public License as
 * published by the Free Software Foundation, either version 3 of the License,
 * or (at your option) any later version. This program is distributed in the
 * hope that it will be useful, but WITHOUT ANY WARRANTY; without even the
 * implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See
 * the GNU Affero General Public License for more details. You should have
...
class Table extends Element {
	...
	function __construct($header0 = null) {
		static $count = 0;
		parent::__construct ( 'table:table', null, Document::NS_TABLE );
	...
	/*
	 * @row	integer|couple	row id or (rowstart,rowend) @col	integer|couple	col
	 * id or (colstart,colend)
	 */
	function put($col, $row, $value) {
		list ( $cmin, $cmax ) = is_array ( $col ) ? $col : array (
				$col, 
				$col 
		);
		...
		for($i = 0; $i <= $cmin; $i ++)
			if (! $c = $this->_columns->getColumn ( $i ))
				$this->_columns->addColumn ();
		list ( $rmin, $rmax ) = is_array ( $row ) ? $row : array (
				$row, 
				$row 
		);
