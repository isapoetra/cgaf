...
use System\Documents\ODF\Dio\Embedder;
use System\Documents\ODF\Dio\Document;
use System\Documents\ODF\Dio\Embeddable;
use System\Documents\ODF\Dio\Element;
class Document extends Element implements Embeddable {
	protected $_type;
	...
	function __construct($type, $embedded = null) {
		static $count = 0;
		parent::__construct('office:document', null, Document::NS_OFFICE);
		$this->_type = $type;
	...
	function _postAppendChild() {
		...
		if ($this->ownerDocument instanceof Embedder) {
			...
			$this->_contentd = new Content($this->_type);
			$this->_metad = new Meta();
			$this->_stylesd = new Styles();
