...
class Cell extends Element {
	...
	function __construct($index = null, $value = null, $rspan = 1, $cspan = 1) {
		parent::__construct('table:table-cell', null, Document::NS_TABLE);
		$this->_index = $index;
		$this->_value = $value;
	...
	function put($value) {
		...
		if ($type) {
			$this->setAttributeNS(Document::NS_OFFICE, 'office:value-type', $type);
	...
	function getAddress() {
		...
		return new CellAddress($table, $column, $row);
	}
}
