<?php
namespace System\Documents\ODF\Dio\Table;
 ...
 */
use System\Documents\ODF\Dio\Style;
...
class Row extends Element {
	...
	function __construct($index = null) {
		static $count = 0;
		parent::__construct('table:table-row', null, Document::NS_TABLE);
		$this->_id = ++$count;
		$this->_index = $index;
		$this->_cells = array();
	}
	function _postAppendChild() {
		...
		$this->_style = $as->addStyle($styleName, Style::FAMILY_TABLE_COLUMN);
	...
	function put($col, $row, $value) {
		...
		for ($i = 0; $i <= $cmin; $i++) {
			if (!isset($this->_cells[$i])) {
				$this->_cells[$i] = ($i == $cmin) ? new Cell($i, null, $rmax - $rmin, $cmax - $cmin) : new Cell();
