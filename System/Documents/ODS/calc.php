<?php
namespace System\Documents\ODS;
class Calc {
	private $fonts;
	private $styles;
	public $sheets;
	...
	private function parse($string) {
		...
		xml_parse($xml_parser, $string, strlen($string));
	...
	private function addCell($sheet, $row, $cell, $value, $type) {
		$this->sheets[$sheet]['rows'][$row][$cell]['attrs'] = array(
				'OFFICE:VALUE-TYPE' => $type,
				'OFFICE:VALUE' => $value);
		$this->sheets[$sheet]['rows'][$row][$cell]['value'] = $value;
	}
	private function editCell($sheet, $row, $cell, $value) {
		$this->sheets[$sheet]['rows'][$row][$cell]['attrs']['OFFICE:VALUE'] = $value;
		$this->sheets[$sheet]['rows'][$row][$cell]['value'] = $value;
	}
	function cells($cell, $value = null) {
		$row = preg_replace('#[^\d]+#', '', $cell) - 1;
		$cell = ord(substr($cell, 0, strlen($cell) - strlen($row))) - 65;
		if ($value === null) {
			return isset($this->sheets[$this->currentSheet]['rows'][$row][$cell]['value']) ? $this->sheets[$this->currentSheet]['rows'][$row][$cell]['value'] : null;
		}
		$this->sheets[$this->currentSheet]['rows'][$row][$cell]['attrs']['OFFICE:VALUE'] = $value;
