...
class XMLUtils
{
    public static function toArray(DOMDocument $doc, DOMNode $oDomNode = null)
    {
        // return empty array if dom is blank
        if (is_null($oDomNode) && !$doc->hasChildNodes()) {
            return array();
        }
        $oDomNode = (is_null($oDomNode)) ? $doc->documentElement : $oDomNode;
        if (!$oDomNode->hasChildNodes()) {
            ...
            foreach ($oDomNode->childNodes as $oChildNode) {
                ...
                $mValue = XMLUtils::toArray($doc, $oChildNode);

                $sKey = ($oChildNode->nodeName{0} == '#') ? 0 : $oChildNode->nodeName;

                $mValue = is_array($mValue) ? $mValue [$oChildNode->nodeName] : $mValue;
                if ($oChildNode->nodeName === '#text') {
