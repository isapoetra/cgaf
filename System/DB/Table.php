...
using ( "System.DB.Query" );

class DBTable extends DBQuery {
	...
	protected $_oldData;
	private $_autoCreateTable = false;
	/**
	 *
	 * @param $appOwner
	 * @param $tableName
	...
	function getAllowedRecords ($uid, $fields = '*', $orderby = '', $index = null, $extra = null) {
		...
		if (! $perms->checkModule($mod, "view", $uid)) {
			if (! count($allow)) {return array();// No access, and no allow overrides, so nothing to show.
			}
		} else {
