...
abstract class HTMLUtils
{
    ...
    static public function renderRating($value, $max = 5)
    {
        $retval = '<ul class="rating">';
        for ($i = 0; $i < $max; $i++) {
            $retval .= '<li class="star rate_' . $i . ($i < $value ? ' on' : '') . '"></li>';
        }
        $retval .= '</ul>';
        return $retval;
    }
                                     ...
                                     $showMessage = true, $msg = null, $attr = null)
    {
        ...
        if ($showMessage) {
            $retval .= '<div id="error-message" class="label label-danger On" style="'
    ...
    {
        $retval = '<div class="form-actions form-group">';
...
JS;
    ...
    public static function renderFormField($title, $id, $value, $attr = null,
                                           ...
                                           $containerClass = '', $groupClass = 'form-group',
                                           $additional = null)
    {
        $renderlabel = true;
        $retval = "";
        ...
        switch ($type) {
            case "checkbox":
                break;
            default:
                $attr = self::mergeAttr($attr, array('class' => 'form-control'));
                break;
        }
        $labelPosition = $labelPosition ? $labelPosition : 'left';

        $containerClass = $containerClass ? $containerClass : '';
        $groupClass = 'form-group ' . ($groupClass == 'form-group' ? '' : $groupClass);
        switch ($type) {
            case "checkbox":
                if ($value) {
