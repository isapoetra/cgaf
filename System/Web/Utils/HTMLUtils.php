...
JS;
  ...
  * [3]; }
  */
  public static function renderTextArea($title, $id, $value=null, $attr = null, $editMode = true) {
    return self::renderFormField ( $title, $id, $value, $attr, $editMode, "textarea" );
  }
  public static function renderDateInput($title, $id, $value=null, $attr = null, $editMode = true,$message =null,$input='date') {
  ...
  private static function renderLabel($for, $title) {
    ...
    $retval .= '<label class="control-label" for="' . $for . '">' . $title . '</label>';
    if (self::$_formMode === self::FORM_MODE_TABLE) {
      $retval .= '</th>';
    }
    return $retval;
  ...
  public static function renderFormField($title, $id, $value, $attr = null, $editMode = false, $type = "text", $labelPosition = 'left',$containerClass='controls',$groupClass='control-group',$additional=null) {
    ...
    switch ($type) {
      case 'htmleditor' :
        if ($editMode) {
          ...
          return self::renderLabel ( $id, $title ) . $editor->Render ( true );
        }
      case 'textarea' :
        if ($editMode) {
    ...
    if ($renderlabel) {
      ...
      if (self::$_formMode === self::FORM_MODE_NORMAL) {
        ...
        $retval = ($labelPosition === 'left' ? $lbl : '') . "<td class=\"$id ui-input-container\" id=\"$id-container\">" . $retval . '</td>' . ($labelPosition === 'right' ? $lbl : '');
      }
    }
    return $retval;
  }
  ...
  public static function renderSelect($title, $id, $items, $selected = null, $nonevalue = '-1', $baseLang = null, $attr = null, $editmode = true, $content = null) {
    ...
    if ($editmode) {
      ...
      foreach ( $items as $item ) {
        ...
        $retval .= "<option $sel value=\"$key\" title=\"" . htmlentities ( $tt ) . "\">" . __ ( $baseLang ? $baseLang . '.' . $value : $value, $value ) . "</option>";
      }
  ...
  public static function renderPagination($currentPage, $pageCount, $dataUrl, $rowperpage = 10) {
    ...
    for($i = 0; $i < $max; $i ++) {
      ...
      $retval .= '<li  class="'.$class.'"><a href="' . $uri . '">'.$title.'</a></li>';
    }
    if ($pageCount > 10) {
      $uri = \URLHelper::addParam ( $dataUrl, "_page=$pageCount&_rp=$rowperpage" );
      $retval .= '</li><a href="' . $uri . '" class="'.$class.'">'.$title.'</a></li>';
    }
    $retval .= '</ul>';
    $retval .= '</div>';
    //$retval .= '</div>';
...
EOT;
  ...
  public static function renderDateRange($configs,$includeTime =true) {
    $retval = '<div>';
    $ds = $configs['start']['value'] ? new \Cdate($configs['start']['value']) : null;
    $retval .= self::renderTextBox($configs['start']['title'], $configs['start']['id'],
       $ds ? $ds->format('m/d/Y') : null,
        array(
            'data-input'=>'date',
    ...
    if ($includeTime) {
        $retval .= self::renderTextBox(null, $configs['start']['id'].'_time',
            $ds->format('h:i:s'),array(
            'data-input'=>'time'
        ));
    }
    $ed = new \Cdate($configs['start']['value']);
    $retval .= self::renderTextBox($configs['end']['title'], $configs['end']['id'],
         $ed ? $ed->format('m/d/Y') : null,array(
        'data-input'=>'date',
        'data-time'=>$includeTime,
        'data-start-date'=>$configs['end']['id']
    ));
    if ($includeTime) {
      $retval .= self::renderTextBox(null, $configs['end']['id'].'_time',
          $ed->format('h:i:s'),array(
          'data-input'=>'time'
      ));
    }
    $retval .='</div>';
...
EOT;
  ...
  public static function renderMarkitupEditor($title,$id,$value=null,$configs =array(),$type="html") {
    ...
    $mark = array(
        ...
        'js/jQuery/plugins/markitup/cgaf/'.$type.'/default/style.css'
    );
