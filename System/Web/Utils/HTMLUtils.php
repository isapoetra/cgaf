...
abstract class HTMLUtils {
	...
	public static function renderBoxed($title, $content, $attr = null, $allowNullContent = false) {
		...
		$retval = '';
		$attr = self::mergeAttr($attr, array('class' => 'ui-widget-content ui-corner-all ui-widget-box'));
	...
	public static function renderFormAction() {
		...
		$retval .= self::renderButton('reset', __('reset', 'Reset'), __('reset.descr', 'Reset this form'), array('class' => 'reset button'), true, 'reset.png');
		$retval .= self::renderButton('submit', __('submit', 'Submit'), __('submit.descr', 'Submit this form'), array('class' => 'submit button'), true, 'submit.png');
		$retval .= '</div>';
		return $retval;
	}
...
JS;
	...
	*/
	...
	public static function renderLink($link, $title, $attr = null, $icon = null, $descr = null) {
		...
		return '<a href="' . $link . '" ' . self::renderAttr($attr) . '><span class="tdl">'.$icon.'<span class="title">' . $title . '</span></a>';
	}
...
EOT;
	...
	public static function renderCoordinate($value, $xfield = 'xcoord', $yfield = 'ycoord', $editmode = true) {
		...
		$retval .= '<tr>';
		$retval .= '<td>' . self::renderTextBox(__('longitude', 'Longitude'), $xfield, $value[$xfield], array('class' => 'required number'), $editmode) . '</td>';
		$retval .= '<td>' . self::renderTextBox(__('latitude', 'Latitude'), $yfield, $value[$yfield], array('class' => 'required number'), $editmode) . '</td>';
	...
	public static function Render($o) {
		...
		if (is_string($o)) {
			...
			if ($o instanceof IRenderable) {
				...
				throw new SystemException('unhandled object type '.get_class($o));
			}
		}
