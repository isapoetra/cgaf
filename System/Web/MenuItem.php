...
class MenuItem extends Object implements IRenderable {
	...
	function __construct($id, $title = null, $action = null, $selected = false, $tooltip = null, $xtarget = null, $showIcon = true) {
		...
		$this->_icon = $this->_icon ? $this->_icon : 'cleardot.gif';
	}
	...
	function Render($return = false) {
		...
		if ($this->_showIcon) {
			...
			if ($icon) {
				...
				$icon = '<div class="warning">icon ' . $this->_icon . 'not found</div>';
			}
		}
		
		$caption = $icon . ($this->_showCaption ? '<span>' . __ ( $this->_title ) . '</span>' : "");
		$act = $this->parseLink ( $this->_action );
		
		$r = new WebLink ( $act, $caption, $this->_targetLink, __ ( $this->_tooltip ) );
		$r->setClass ( $this->_css . ' ' . ($this->_selected ? "selected" : "") . (count ( $this->_childs ) ? ' menuparent' : '') );
