...
class TJQUpload extends JQControl {
	...
	function RenderScript($return = false) {
		...
		$retval = '<div id="'.$this->getContainerId().'">'
		.' <div id="'.$this->getConfig('queueID').'">&nbsp;</div>'
		.'<input type="file" name="'.$id.'" id="'.$id.'" />'
		.'<p><a href="javascript:$(\'#'.$id.'\').uploadifyClearQueue()">Cancel All Uploads</a></p>'
		.'<div class="preview" style="display:none">&nbsp;</div></div>';
		return $retval;
	}
}