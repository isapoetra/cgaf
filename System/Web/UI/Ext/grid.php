...
class Grid extends Control {
	...
	function setViewReport($value) {
		if ($value && in_array("reports", $this->_showbutton)) {
			...
			$js = <<< EOT
function(a,b){
	var grid=Ext.getCmp('{$this->getId()}');
	if (grid) {
		sm=grid.getSelectionModel().getSelected();
		if(!sm){
			Ext.MessageBox.alert('Error','No Data Selected');
			return false;
		}
		doLoadPageContent('{$this->_reportURL}&'+ genFid('{$this->_fieldId}',sm.data) );
	}
}"
EOT;
	...
	function assignStandard($m = null, $keyid = null) {
		...
		$m = $m ? $m : Request::get("__c");
		$c = $this->getAppOwner()->getController($m);
		if (!$c) {
			return null;
		}
...
function(a,b){
	...
	if (grid) {
		...
		doCreateRemoteWindow({{$winc}
			autoLoad:{
				url:'{$this->_addEditURL}&'+ genFid('{$this->_fieldId}',sm.data)
			}
		},null,{$this->_winCallBack});
	}
}
EOT;
			$this->addTopBar(array(
...
class JExtGridCheckboxSelectionModel extends CustomComponent {
