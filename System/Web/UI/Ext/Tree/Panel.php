...
class Panel extends Control {
	...
	protected $_showButton = array("add",
		"edit",
		"delete",
		"view",
		"reports");
	...
	function __construct($configs) {
		...
		$this -> _controlScript = array("id" => "tree",
			"url" => "tree.js");
		$this -> addIgnoreConfigStr(array("root"));
		...
		if(!isset($configs["root"])) {
			$configs["root"] = new JExtCustom("Ext.tree.AsyncTreeNode", array("text" => 'root',
				"id" => "0",
				"expanded" => true));
		}
		$this -> setConfigs(array("trackMouseOver" => true,
			"useArrows" => true,
			"enableDD" => true,
			"animate" => true,
			"height" => "400",
		...
		if($this -> _renderStandardButton) {
			...
			if(!$removed) {
				if($perms -> checkModule($m, "add") && in_array("add", $this -> _showButton)) {
					...
					$buttons[] = array("text" => "new",
						"iconCls" => JExt::ICON_ADD,
						"handler" => $add);
					$buttons[] = array("text" => "add Child",
						"iconCls" => JExt::ICON_ADD . "child",
						"handler" => "function(){ var tree=Ext.getCmp('{$this->getId()}'); s = tree.getSelectionModel().getSelectedNode();if (s) { doCreateRemoteWindow({autoLoad:{url:'{$this->_addEditURL}&parent_id='+s.id}" . ($winc ? ",$winc" : "") . "},'$renderto',{changed:function(){tree.loader.load(s.parentNode)} })}}");
					$buttons[] = new JExtJS("new Ext.menu.Separator()");
				}
				if(in_array("view", $this -> _showButton)) {
					$winc = $this -> getWinConfig("view");
					$buttons[] = array("text" => "view",
						"iconCls" => JExt::ICON_VIEW,
						"handler" => "function(){ tree=Ext.getCmp('{$this->getId()}');s = tree.getSelectionModel().getSelectedNode();if (s) { doCreateRemoteWindow({autoLoad:{url:'{$this->_viewURL}&$id='+s.id},modal:true,autoScroll:true" . ($winc ? "," . $winc : "") . "},'$renderto')}}");
					$action = $this -> _viewMode == "window" ? "doCreateRemoteWindow({autoLoad:{url:'{$this->_viewURL}&$id='+node.id},modal:true,autoScroll:true" . ($winc ? "," . $winc : "") . "},'$renderto')" : "doLoadPageContent('{$this->_viewURL}&$id='+node.id);";
				...
				if($perms -> checkModule($m, "edit") && in_array("edit", $this -> _showButton)) {
					...
					$buttons[] = array("text" => "edit",
						"iconCls" => JExt::ICON_EDIT,
						"handler" => "function(){ tree=Ext.getCmp('{$this->getId()}');s = tree.getSelectionModel().getSelectedNode();if (s) { doCreateRemoteWindow({autoLoad:{url:'{$this->_addEditURL}&$id='+s.id},modal:true,autoScroll:true" . ($winc ? "," . $winc : "") . "},'$renderto',{changed:function(){tree.loader.load(s.parentNode)} });}}");
				}
				if($perms -> checkModule($m, "delete") && in_array("delete", $this -> _showButton)) {
					$buttons[] = array("text" => "delete",
						"iconCls" => JExt::ICON_DELETE,
						"handler" => "function(){G.confirm('" . __("Delete This Data") . "', function(){tree=Ext.getCmp('{$this->getId()}');s = tree.getSelectionModel().getSelectedNode();if (s) { G.loadUrl('{$this->_deleteURL}&del='+s.id,{success:function(e){tree.loader.load(s.parentNode)}})}})}");
				}
				if(in_array("reports", $this -> _showButton) && in_array("reports", $this -> _showButton)) {
					$this -> _config["tbar"][] = array("text" => 'Reports',
			...
			if($this -> getConfig("enableDD") && $this -> _reorderAction) {
				$this -> addEvent("beforemovenode", "function(tree, node, oldParent, newParent, index ) { G.confirm('Move Node?',function(){ Ext.Ajax.request({method:'GET',url:tree.loader.baseUrl+'&_a={$this->_reorderAction}',scope:this,params:{id:node.id,newparent:newParent.id,index:index}})});} ");
			}
		}
