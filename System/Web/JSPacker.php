...
class JSPacker {
  ...
  public function __construct ($_script = null, $_encoding = 62, $_fastDecode = true, $_specialChars = false) {
    $this->_script = $_script . "\n";
    if (array_key_exists($_encoding, $this->LITERAL_ENCODING)) $_encoding = $this->LITERAL_ENCODING[$_encoding];
  ...
  public function clear () {
    $this->_script = "";
    $_encoding = $this->LITERAL_ENCODING[62];
    $this->_encoding = min((int) $_encoding, 95);
    $this->_fastDecode = true;
  ...
  private function _encodeKeywords ($script) {
    ...
    if ($this->_encoding > 62) $script = $this->_escape95($script);
    // create the parser
    $parser = new ParseMaster();
  ...
  private function _analyze ($script, $regexp, $encode) {
    ...
    if (! empty($all)) {
      ...
      do {
        if (! isset($_sorted[$i])) $_sorted[$i] = substr($unsorted[$j ++], 1);
  ...
  private function _bootStrap ($packed, $keywords) {
    ...
    if ($this->_fastDecode) {
      ...
      if ($count == 0) $decode = preg_replace($this->_safeRegExp('($count)\\s*=\\s*1'), '$1=0', $decode, 1);
    }
    // boot function
  ...
  private function _encode95 ($charCode) {
    $res = '';
    if ($charCode >= $this->_encoding) $res = $this->_encode95($charCode / $this->_encoding);
