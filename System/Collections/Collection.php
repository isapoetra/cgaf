...
class Collection extends Object implements IteratorAggregate, ArrayAccess, Countable {
	...
	public function setItemClass ($value) {
		...
		if (! class_exists($value, false)) throw new SystemException("Class $value not Found");
		$this->_iClass = $value;
	}
	...
	public function itemAt ($index) {
		...
		else throw new InvalidOperationException('list_index_invalid', $index);
	}
	...
	public function add ($item, $multi = false) {
		...
		if ($this->_iClass) {
			if ( (get_class($item) !== $this->_iClass) && (! is_subclass_of($item, $this->_iClass))) throw new SystemException("Item Not Valid Required class $this->_iClass given class " . get_class($item));
		}
	...
	public function insertAt ($index, $item) {
		if (! $this->_r) {
			...
			$this->_d[$this->_c ++] = $item;
				...
				array_splice($this->_d, $index, 0, array(
				$item));
				$this->_c ++;
			} else
	...
	function assign ($var, $val =null) {
		...
		} else if ($var !== null) throw new SystemException('list_data_not_iterable');
	}
	...
	public function mergeWith ($data) {
		if (is_array($data) || ($data instanceof Traversable)) {
			...
			$this->add($item);
		} else if ($data !== null) throw new InvalidOperationException('list_data_not_iterable');
	}
