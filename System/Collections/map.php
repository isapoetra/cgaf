<?php
class TMap extends Object implements IteratorAggregate, ArrayAccess, Countable {
  /**
  ...
  public function getIterator () {
    return new TGMapIterator($this->_d);
  }
  ...
  public function add ($key, $value) {
    if (! $this->_r)
      $this->_d[$key] = $value;
    else throw new InvalidOperationException('map_readonly', get_class($this));
    return $value;
  }
   ...
   */
  public function assign ($obj, $multi = false) {
    if (is_array($obj) || $obj instanceof Traversable) {
      if ($this->getCount() > 0) $this->clear();
      foreach ($obj as $key => $value)
        $this->add($key, $value);
    } else if ($obj !== null) throw new InvalidDataTypeException('map_data_not_iterable');
  }
  ...
  public function mergeWith ($data) {
    if (is_array($data) || $data instanceof Traversable) {
      foreach ($data as $key => $value)
        $this->add($key, $value);
    } else if ($data !== null) throw new InvalidDataTypeException('map_data_not_iterable');
  }
 ...
 * @since 3.0
 */
class TGMapIterator implements Iterator {
  /**
