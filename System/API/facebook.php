...
abstract class BaseFacebook
{
    ...
    {
        return $this
            ->getUrl('www', 'extern/login_status.php',
                array_merge(
                    array(
    ...
    protected function getAccessTokenFromCode($code, $redirect_uri = null)
    {
        ...
        try {
            ...
            $access_token_response = $this
                ->_oauthRequest($this->getUrl('graph', '/oauth/access_token'),
                    $params = array(
                        'client_id' => $this->getAppId(),
...
class fbInstance extends BaseFacebook
{
...
EOT;
    ...
    function send($config)
    {
        ...
        $config = Utils::arrayMerge($x, $config);
        if (!array_key_exists('href', $config)) {
            return null;
        }
    ...
    function comments($config)
    {
        $ori = array(
            'num_post' => 2,
            'width' => 500);
        $config = Utils::arrayMerge($ori, $config);
        if (!array_key_exists('href', $config)) {
            return null;
        }
    ...
    {
        return $this->getAppOwner()->renderView('auth/fb/status', null, array(
            'fb' => self::getfbInstance()), 'auth');
        /*
        $def = array(
            'show-faces' => true,
            'max-rows' => true,
