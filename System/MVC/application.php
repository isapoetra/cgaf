...
if (System::isWebContext ()) {
	...
	class Application extends WebApplication {
		...
		protected function initRequest() {
			...
			if (! Request::isDataRequest () && ! $this->getVars ( 'title' )) {
				...
				$deftitle = $this->getAppId () === \CGAF::APP_ID ? \CGAF::getConfig ( 'cgaf.description' ) : $this->getConfig ( 'app.title', $this->getAppName () );
								...
								if ($this->getConfig ( 'content.rendercontentaction' )) {
									...
									if ($ctl->isAllow ( ACLHelper::ACCESS_MANAGE )) {
										$haction [] = HTMLUtils::renderLink ( URLHelper::addParam ( $this->getAppUrl (), array (
												'__c' => $row->controller,
												'__a' => 'aed'
										) ), __ ( $row->controller . '.add.title', 'Add' ), null, 'icons/add.png', __ ( $row->controller . '.add.descr', 'Add Data' ) );
									}
								}
		...
		function getUserInfo($id) {
			...
			$this->_userInfo [$id] = new \UserInfo ( $this, $id );
			return $this->_userInfo [$id];
		}
	...
	using ( 'System.Applications.console' );
