...
abstract class Application extends AbstractApplication {
  ...
  */
  ...
  protected function handleRequest() {
    ...
    if (!$content) {
      if ($controller && $controller->isAllow($action)) {
        ...
        if (!$content) {
          $content = $controller->getLastError() ? $controller->getLastError()
            : "access to action $action is denied on controller " . Request::get('__c');
        }
      }
    }
    //$retval = '';
  ...
  function renderContent(
    $location, $controller = null, $returnori = false, $return = true, $params = null, $tabMode = false,
    $appId = null) {
    if ($controller === null) {
      $controller = $this
        ->getController()
