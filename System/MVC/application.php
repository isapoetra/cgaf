...
abstract class Application extends AbstractApplication {
	...
	private function getControllerInstance($controllerName) {
		if (!is_string($controllerName))
			throw new InvalidDataTypeException(
					'string exprected,' . typeof($controllerName) . ' given');
	...
	protected function initRequest() {
		...
		if (!Request::isDataRequest() && !$this->getVars('title')) {
			...
			$deftitle = $this->getAppId() === \CGAF::APP_ID ? \CGAF::getConfig(
							'cgaf.description')
					: $this->getConfig('app.title', $this->getAppName());
			$this
					->Assign('title',
	...
	function renderMenu($position, $controller = true, $selected = null,
			$class = null, $renderdiv = true) {
		if ($controller) {
			...
			$retval = "";
			if ($renderdiv) {
				$retval = '<div class="menu-container" id="menu-container-'
						. $position . '" data-role="navbar">';
			}
			$menu = new Menu();
			if ($this->getConfig('app.designmode', CGAF_DEBUG)) {
				$items[] = new MenuItem('menu-design', 'Design',
						'/menus/manage/' . $position);
			}
				...
				if ($items) {
					foreach ($items as $k => $row) {
						...
						if (($row->getActionType() == 1
								|| $row->getActionType() == null)) {
							$action = explode('/', $action);
							if (isset($action[1]) && $action[0] === $rname
									&& $action[1] === $a) {
								$row->setSelected(true);
							} elseif (!isset($action[1])
									&& $action[0] === $rname && $a === 'index') {
								$row->setSelected(true);
							}
						}
						$items[$k] = $row;
					...
					if ($row->controller !== null
						...
						if (!$ctl) {
							...
							if (class_exists($c, false)) {
								$ctl = new $c($this); //$this->getClassInstance($row->controller, "Controller", $this);
							}
						}
					}
				} catch (\Exception $e) {
