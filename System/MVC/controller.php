...
abstract class Controller extends \BaseObject implements IController, \ISearchProvider {
	...
	protected function getAction($o, $id = null, $route = null, $params = null) {
		...
		if (!$this->_renderInternalAction || !$this->getConfig('renderaction.'.$appRoute['_a'], true))
			return $retval;
		...
		if ($controller->isAllow(ACLHelper::ACCESS_MANAGE)) {
			if ($o) {
				...
				if ($croute['_a'] !== 'aed' && $this->_isAllowAction($id, 'add')) {
					$retval[] = new Anchor(\URLHelper::Add($url, 'aed', $params), __('action.add'), HTMLUtils::mergeAttr($this->_actionAttr, array(
							'class' => 'icon-plus'
					)));
				}
				if ($this->isAllow('manage')) {
					$retval[] = new Anchor(\URLHelper::Add($url, 'manage', $params), __('action.manage'), HTMLUtils::mergeAttr($this->_actionAttr, array(
							'class' => 'icon-list-alt'
					)));
				}
			}
		...
		if ($id) {
			if ($this->isAllow('detail') && $croute['_a'] !== 'detail') {
				$retval[] = new Anchor(\URLHelper::Add($url, 'detail', 'id=' . (is_array($id) ? implode(',', $id) : $id)), __('action.detail'), HTMLUtils::mergeAttr($this->_actionAttr, array(
						'class' => 'icon-book'
				)));
			}
			if ($appRoute['_c'] === $croute['_c'] && $this->isAllow('edit') && $this->_isAllowAction($id, 'edit') && (!$id || $croute['_a'] !== 'aed')) {
				$retval[] = new Anchor(\URLHelper::Add($url, 'aed', 'id=' . $id), __('action.edit'), HTMLUtils::mergeAttr($this->_actionAttr, array(
						'class' => 'icon-edit'
				)));
			}
			if ($appRoute['_c'] === $croute['_c'] && $this->isAllow('delete') && $this->_isAllowAction($id, 'delete')) {
				$retval[] = new Anchor(\URLHelper::Add($url, 'del', 'id=' . $id), __('action.delete'), HTMLUtils::mergeAttr($this->_actionAttr, array(
						'class' => 'icon-trash'
				)));
			}
		}
	...
	function detail($args = null, $return = null) {
		...
		if ($id !== null) {
			$row = $m->reset('detail')->whereId($id)->loadObject();
			$args['row'] = $row;
		} else {
	...
	function search($s, $config) {
		...
		return $this->renderView('search-result', array(
						'rows' => $rows,
						'row' => $row
				));
	}
	...
	public function getMenuItems($position, $parent = 0, $actionPrefix = null, $showIcon = true) {
		$rows = $this->getAppOwner()->getMenuItems($position, $parent, $actionPrefix, $showIcon, false, false, $this->getRouteName());
		$filtered = array();
		if (count($rows)) {
	...
	protected function renderStaticConent($a, $f, $template = null, $params = null) {
